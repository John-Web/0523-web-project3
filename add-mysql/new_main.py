...
main_re = "insert into main(" \
          ...
          " title, poster, imdb, year, duration, casts, countries, directors, genres, languages, pubdate, writers)" \
    ...
    for i in range(0, 10000):
        ...
        if len(j['rating']['stars']):
            main_text.append((
                ...
                json.dumps(j['casts']), json.dumps(j['countries']), json.dumps(j['directors']), json.dumps(j['genres']),
                json.dumps(j['languages']), json.dumps(j['pubdate']), json.dumps(j['writers'])
            ))
        else:
            main_text.append((
                j['_id'], j['rating']['average'], j['rating']['rating_people'],
